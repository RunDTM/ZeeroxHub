...
function Flux:Window(text, bottom,mainclr,toclose)
	...
	TabHold.ScrollBarImageColor3 = Color3.fromRGB(42, 42, 42)
	...
	function Tabs:Tab(text)
		...
		TabTitle.Position = UDim2.new(0.05, 0, 0.25, 0)
		...
		MakeDraggable(Container,MainFrame)
		...
		function ContainerContent:Toggle(text, desc,default, callback)
			...
			local function ToggleFunction()
				...
				Toggled = not Toggled
				spawn(function()
					local succ,err = pcall(callback, Toggled)
					if not succ then
						warn(err)
					end
				end)
		...
		function ContainerContent:Dropdown(text,list,callback)
			...
			DropItemHolder.ScrollBarThickness = 10
			...
			for i,v in next, list do
				...
				Item.Size = UDim2.new(0, 427, 0, 20)
		...
		function ContainerContent:Multidropdown(text,list,callback)
			...
			local selecteditems = {}
			local Selected = text
			for i2,v2 in next,list do
				if v2[2] then
					table.insert(selecteditems, v2[1])
				end
			end
			Selected = table.concat(selecteditems, ', ')
			local FrameSize = 43
			local ItemCount = 0
			local DropToggled = false
			local Dropdown = Instance.new("TextButton")
			...
			for i,v in next, list do
				...
				Item.MouseButton1Click:Connect(function()
					...
					Selected = table.concat(selecteditems, ', ')
					pcall(callback, list, selecteditems)
					Title.Text = text
					DropToggled = not DropToggled
					Dropdown:TweenSize(UDim2.new(0, 457, 0, 43), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, .6, true)
					TweenService:Create(
